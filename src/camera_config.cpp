...
void CameraConfig::Init() {
    ...
    while (in.read_row(model_name, device_code, stream_url)) {
        if (!(model_name.empty() || device_code.empty() || stream_url.empty())) {
            ...
            SPD_ERROR("camera config error, model_name: {}, device_code: {}, stream_url: {}", model_name, device_code, stream_url);
        }
    }
    ...
    for (const auto &camera_info : camera_list_) {
        SPD_INFO(
            "camera config, model_name: {}, device_code: {}, stream_url: {}", camera_info.yolo_model_name, camera_info.device_code, camera_info.stream_url);
    }
}
